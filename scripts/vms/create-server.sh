...
echo "  RAM: 4096 MB"
echo "  Disco: 30 GB"
echo "  Red 1: VM Network (Internet/Gestión)"
echo "  Red 2: M_vm's (VMs Internas IPv6)"
echo "  Red 3: M_s's (Red Física/Switch)"
...
if [ $? -eq 0 ]; then
    ...
    echo "   - Usuario: ubuntu"
    echo "   - Configurar red durante instalación"
    echo ""
    echo "3. Configurar las 3 interfaces de red:"
    echo "   - ens33: DHCP (Internet/Gestión)"
    echo "   - ens34: fd00:cafe:cafe::1/64 (VMs Internas)"
    echo "   - ens35: DHCP o IP estática (Red Física)"
    echo ""
    echo "4. Desde el servidor Ubuntu, ejecutar:"
    echo "   ${BLUE}bash scripts/server/setup-server.sh${NC}"
    echo ""
    echo "5. Esto configurará:"
    echo "   - Paquetes base"
    echo "   - Red IPv6 (3 interfaces) y NAT64"
    echo "   - DNS (BIND9 + DNS64)"
    echo "   - DHCPv6"
